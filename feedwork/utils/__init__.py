...
if HR_RUN_PROD == "y":
    ...
    HR_LOG_FILE_LEVEL = _os.getenv("HR_LOG_FILE_LEVEL", "INFO")
    if HR_LOG_FILE_LEVEL not in ("DEBUG", "INFO", "WARNING", "ERROR"):
        raise ValueError(f'Environment HR_LOG_FILE_LEVEL must be ("DEBUG", "INFO", "WARNING", "ERROR")') from None
    ...
    handlers = [{"sink": HR_LOG_FILE, "format": format_str,
                 "level": HR_LOG_FILE_LEVEL, "rotation": "00:00", "retention": "1 days"}]
    if HR_LOG_CONSOLE == "y":
        handlers.append({"sink": _sys.stdout, "level": HR_LOG_FILE_LEVEL, "format": format_str})
