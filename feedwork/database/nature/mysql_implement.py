...
class MYSQLImplement(DatabaseInterface):

    def create_connection(self, db_host: str, db_port: int, db_name: str, username: str, password: str, dsn):
        # 若在这里设置事务，那么该连接在使用过程中无法再设置事务
        conn = pymysql.connect(host=db_host, port=db_port, user=username, passwd=password, db=db_name,
